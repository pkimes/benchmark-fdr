...
This file contains Supplementary Figures to complement the main results figures 
in `manuscript_figures_main.Rmd`. 
...
```{r FigureS1, fig.width = 10, fig.height = 8}
...
# Figure S1A: Not all methods control FDR at nominal alpha level
p1 <- plotsim_average(res5d, met="FDR",filter_set = excludeSet,
                merge_ihw = TRUE, errorBars=TRUE) +
      theme(legend.position="none") +  
      ggtitle("FDR by alpha level") +
      scale_x_continuous("alpha cutoff", breaks=seq(0, 1, by=0.02))

# Figure S1B: TPR of methods across nominal alpha level
p2 <- plotsim_average(res5d, met="TPR",filter_set = excludeSet,
                merge_ihw = TRUE, errorBars=TRUE) +
      theme(legend.position="none")  +  
      ggtitle("TPR by alpha level") +
      scale_x_continuous("alpha cutoff", breaks=seq(0, 1, by=0.02))

# Figure S1C: ROC curve
p3 <- plotsim_average(res5d, met=c("FPR", "TPR"),filter_set = excludeSet,
                merge_ihw = TRUE, errorBars=TRUE) +
      theme(legend.position="none") +
      ggtitle("ROC (FPR vs TPR)") +
      scale_x_continuous("FPR", breaks=seq(0, 1, by=0.01))

# Figure S1D: Some methods are tailored to detecting smaller effect sizes
p4 <- covariateLinePlot(de5, alpha=alpha.thresh, covname="log2FC", trans="log1p",
                        nbins=25)
leg <- get_legend(p4)
p4 <- p4 +
  ylab("Rejection rate") +
...
# Figure S1E - upsetr plot - save separately
pdf(file.path(outdir, "FigureS1E.pdf"), width = 8, height = 4, onefile=FALSE)
aggupset(de5, alpha=alpha.thresh, supplementary = FALSE, 
                       return_list = FALSE, nintersects = 12)
grid.text("Mean overlap of rejection sets", x = 0.65, y = 0.97, 
...
# Figure 2 (ALL)
FigS1 <- plot_grid(p1, p2, p3, p4, ncol = 2, 
          labels = LETTERS[1:4], label_size = 20, align = "hv")
FigS1 <- plot_grid(FigS1, leg, ncol = 2, rel_widths = c(1, 0.2))
FigS1
ggsave(file.path(outdir, "FigureS1A-D.pdf"), width=8.75, height=6)

```

# Figure SR1 (added in response to reviewer comments)
...
r1.1 <- plot_grid(r1.1, leg, rel_widths = c(1, .2)) 
r1.1
ggsave(file.path(outdir, "Figure_SR1.pdf"), width = 8, height = 3)
```


# Figure S2

This Figure contrasts FDR results in different yeast simulation settings.
...
if (file.exists("yeast_summary_tab_all.rds")){
  tab <- readRDS("yeast_summary_tab_all.rds")
}else{
  tab <- NULL
  ...
  saveRDS(tab, file = "yeast_summary_tab_all.rds")
}
...
ggsave(file.path(outdir, "FigureS2.pdf"), width=10, height=12)

```

# Figures S3-S8

Figures S3-S8 are created in `manuscript_figures_sims.Rmd`.

# Figures S9

Here we'll contrast the results of informative and uninformative covariates 
on the yeast simulation (DE sample size 5). We'll plot
...
if (file.exists("yeast_summary_tab.rds")){
  tab <- readRDS("yeast_summary_tab.rds")
}else{
  tab <- data.frame()
  for (l in seq_along(resfile)){
    x <- readRDS(resfile[l])
  ...
  saveRDS(tab, file = "yeast_summary_tab.rds")
}
...
Fig <- plot_grid(FigA, FigB,
                 ...
                 hjust = 0, axis = "rlbt")
plot_grid(Fig, leg, ncol=2, rel_widths = c(1,0.1))
ggsave(file.path(outdir, "FigureS9.pdf"), width=10, height=12)
```

# Figures S10 & S16

These figures explore the informativeness of covariates in case studies. 
...
null <- which(grepl(c("goodrich|papa|baxter"), objects) & 
              grepl(c("otu"), objects) | 
              grepl("log", objects) | 
              grepl("so4", objects))
...
tab <- tidy_df(objects=objects, colLabels=filename, 
              fill="meanRank", annotate=NULL, alpha = alpha.thresh) %>%
...
log_neg_percent <- function()
              scales::trans_new("log_neg_percent", 
                ...
                breaks = function(x) {
                  if (max(x) > 200){
                    c(-100, -25, -5, 0, 5, 25, 100, 300)
                  }else if (max(x) > 100){
                    c(-100, -25, -5, 0, 5, 25, 100, 200)
                  }else{
                    c(-100, -25, -5, 0, 5, 25, 100)
                  }
                },
                format = function(x) {
                  paste0(x,"%")
...
Fig_impactofinfcov <- ggplot(dftab, aes(x = dataset, y = method, fill = diff_inf_percent*100)) + 
        ...
        scale_fill_gradientn("% Change in\nrejections",
                             ...
                             limits = c(min(c(-20, botRange)), topRange),
                             values = scales::rescale(c(-transform(max(20,abs(botRange))), 
                                                        0, transform(topRange))), 
        ...
        theme(axis.text.x = element_text(angle = 90, vjust = 1/2, hjust = 1),
              legend.position = c(0.995, 0.95), legend.justification = c(1, 1),
...
Fig_impactofinfcov
ggsave(file.path(outdir, "FigureS16.pdf"), width = 15.5, height = 4.25)
...
if(file.exists("yeast_tab_covimpact.rds")){
  tab <- readRDS("yeast_tab_covimpact.rds")
}else{
  tab <- data.frame()
  for (l in seq_along(resfile)){
    x <- readRDS(resfile[l])
  ...
  save(tab, file="yeast_tab_covimpact.rds")
}
...
ggsave(file.path(outdir, "FigureS10.pdf"), width=9.5, height=5)

```

# Figure S11

Figure S11 is created in `manuscript_figures_sims.Rmd`.
...
```{r, FigureS12, fig.width = 9, fig.height = 7}
...
pA <- covariateLinePlot(res, alpha=alpha.thresh, covname="ind_covariate", trans="log1p",
                        nbins=25) +
...
assayNames(res) <- "qvalue"
res <- addDefaultMetrics(res)
pB <- covariateLinePlot(res, alpha=alpha.thresh, covname="detection", trans="log1p",
                        nbins=25) +
...
assayNames(res) <- "qvalue"
res <- addDefaultMetrics(res)
pC <- covariateLinePlot(res, alpha=alpha.thresh, covname="ind_covariate", trans="log1p",
                        nbins=25) +
...
assayNames(res) <- "qvalue"
res <- addDefaultMetrics(res)
pD <- covariateLinePlot(res, alpha=alpha.thresh, covname="ind_covar_AF", trans="log1p",
                        nbins=25) +
  ggtitle("GWAS case study") +
  xlab("Minor Allele Frequency (MAF) percentile") +
  theme(legend.position="none")

ps12 <- plot_grid(pA, pB, pC, pD, ncol=2, labels = LETTERS[1:4])
ps12 <- plot_grid(ps12, leg, ncol=2, rel_widths = c(1,0.2))
ps12
ggsave(file.path(outdir, "FigureS12.pdf"), width=9, height=7)
```
...
We'll do so for the same 4 case studies as in figure S12
...
by of number of rejections @ FDR cutoff `r alpha.thresh`.

```{r, FigureS14, fig.width = 9, fig.height = 7}
library(SummarizedBenchmark)
library(stringr)
...
FigS14All.rowOrder <- c("bonf", "bh", "ihw", "qvalue", "bl", "adapt-glm", "lfdr",
                     "fdrreg-e", "fdrreg-t", "ashq")
FigS14All <- plotMethodRanks(objects, colLabels = casestudy, alpha = alpha.thresh,
                        excludeMethods = NULL, rowOrder = FigS14All.rowOrder) +
        theme(axis.text.x = element_text(angle = 90, vjust = 1/2, hjust = 1),
...
Extract a table of the values in Figure S14.

```{r}
# Supplementary table S3 
library(xtable)
tabS3 <- plotMethodRanks(objects, colLabels = casestudy, alpha = alpha.thresh,
                        tableOnly = TRUE, excludeMethods = NULL)
tabS3 <- tabS3 %>% 
  ...
  mutate(method =  factor(method, levels = rev(FigS14All.rowOrder))) %>%
  arrange(method)    

print(xtable(tabS3), include.rownames=FALSE)

# generate detailed case study table for creating sim vs cs ranks boxplots
casestudy.tab <- plotMethodRanks(objects, colLabels = dataset, alpha = alpha.thresh,
                                 tableOnly = TRUE)
```
...
pfdr <- plotMethodRanks(objs, 
                colLabels = c("sim", "sim"), 
                alpha = alpha.thresh, xlab = "Comparison",
                fill = "FDR",
                rowOrder = FigS14All.rowOrder, 
                annotate = "FDR", excludeMethods = NULL,
                maxColor = "black") +
          scale_fill_gradient2(high = "darkred", 
                         mid = "white", 
                         low = "#2171B5", 
                         midpoint = alpha.thresh) +
          labs(fill = "Mean FDR") +
          theme(axis.text.x = element_text(angle = 90, vjust = 1/2, hjust = 1),
              legend.justification = c(1, 0.5),
              legend.text = element_text(size = 7),
...
ggdraw() +
  ...
  draw_plot(FigS14All, 0, 0, 0.52, 1) +
  draw_plot_label(c("A", "B"), c(0, 0.53), c(1, 1), size = 15) +
  draw_plot_label("Simulation", 0.69, 0.06, size =11, fontface = "plain")
ggsave(file.path(outdir, "FigureS14.pdf"), width=9.5, height=5)

# generate detailed yeast sim table for creating sim vs cs ranks boxplots
yeastsim.tab <- plotMethodRanks(objs, colLabels = labs, alpha = alpha.thresh,
                                 tableOnly = TRUE)
```
...
ggsave(file.path(outdir, "FigureS15.pdf"), width=6, height=4)
