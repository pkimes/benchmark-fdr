...
```{r SFigureSim1, fig.width = 12, fig.height = 15}
...
SFigS1
ggsave(file.path(outdir, "SFigureSim1.pdf"), width = 12, height = 15)
```
...
```{r SFigureSim2, fig.width = 12, fig.height = 15}
...
SFigS2
ggsave(file.path(outdir, "SFigureSim2.pdf"), width = 12, height = 15)
```
...
```{r SFigureSim3, fig.width = 12, fig.height = 15}
...
SFigS3
ggsave(file.path(outdir, "SFigureSim3.pdf"), width = 12, height = 15)
```
...
```{r SFigureSim5, fig.width = 8, fig.height = 6}
## SFigure Sim5 B,C: FDR/TPR of methods across unimodal effect size distribution
ps5b <- genplot(ua25met, cov = alpha, met = "FDR") +
    facet_grid(. ~ signal, labeller = label_parsed) +
    scale_x_continuous(breaks = seq(0, .1, .02)) +
    expand_limits(x = 0)
ps5c <- genplot(ua25met, cov = alpha, met = "TPR") +
    facet_grid(. ~ signal, labeller = label_parsed) +
    scale_x_continuous(breaks = seq(0, .1, .02)) +
    expand_limits(x = 0)

## SFigure Sim5 D,E: FDR/TPR difference of methods
ps5d <- genplot(ua25met, cov = alpha, type = "diff", met = "FDR") +
    facet_grid(. ~ signal, labeller = label_parsed) +
    scale_x_continuous(breaks = seq(0, .1, .02)) +
    expand_limits(x = 0) +
    ylab(expression(paste("Mean ", Delta, "FDR (informative-uninformative)")))
ps5e <- genplot(ua25met, cov = alpha, type = "diff", met = "TPR") +
    facet_grid(. ~ signal, labeller = label_parsed) +
    scale_x_continuous(breaks = seq(0, .1, .02)) +
    expand_limits(x = 0) +
    ylab(expression(paste("Mean ", Delta, "TPR (informative-uninformative)")))
...
## pull SFigure Sim5 together
SFigS5 <- plot_grid(ps2a + guides(color = FALSE, linetype = FALSE),
                    ...
                    labels = LETTERS[1:5], ncol = 1, rel_heights = c(.6, rep(1, 4)))
## SFigS5 <- plot_grid(ps5Title, SFigS5, ncol = 1, rel_heights = c(0.03, 1))
SFigS5 <- plot_grid(SFigS5, get_legend(ps5b), rel_widths = c(6.5, 1))
SFigS5
ggsave(file.path(outdir, "SFigureSim5.pdf"), width = 12, height = 15)
```


# Supplementary Figure (informativeness covariates)
...
```{r SFigureSim6, fig.width = 12, fig.height = 4}
## SFigure Sim6 A: Informativeness covariate relationships (continuous)
...
ps6a <- ggplot(cinfovar, aes(x = x, y = pi0)) +
    geom_line() +
    expand_limits(y = c(0.5, 1)) + 
    theme_classic() +
    scale_x_continuous(breaks = seq(0, 1, .2)) + 
...
## SFigure Sim6 B: Informativeness covariate relationships (discrete)
...
ps6b <- ggplot(dinfovar, aes(x = x, y = pi0)) +
    geom_line() +
    expand_limits(y = c(0.5, 1)) + 
    theme_classic() +
    scale_x_continuous(breaks = seq(0, 1, .2)) + 
...
## pull SFigure Sim6 together
SFigS6 <- plot_grid(ps6a, ps6b, labels = LETTERS[1:2], ncol = 1)
SFigS6
ggsave(file.path(outdir, "SFigureSim6.pdf"), width = 12, height = 4)
```
