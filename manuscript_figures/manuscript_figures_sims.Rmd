...
```{r, fig.width = 15, fig.height = 7.5}
gp <- plot_grid(pfdr + guides(color = FALSE, linetype = FALSE),
...
## trimmed ranges
gp_zoom <- plot_grid(pfdr + coord_cartesian(ylim = c(0, .25)) + guides(color = FALSE, linetype = FALSE),
                     ptpr, rel_widths = c(1, 1.25), labels = LETTERS[1:2])
gp_zoom

ggsave(file.path(outdir, "FigureSsim-nullprop-zoom.pdf"), gp_zoom, width = 10, height = 5)
```
...
```{r, fig.width = 15, fig.height = 7.5}
gp <- plot_grid(pfdr + guides(color = FALSE, linetype = FALSE), ptpr,
          rel_widths = c(1, 1.25), labels = LETTERS[3:4])
gp
