...
dplyr::filter(res_nullset, key == "rejectprop") %>%
    ...
    ggplot(aes(x = Method, y = mean_value)) +
...
```{r, fig.width = 8, fig.height = 3}
gp <- dplyr::filter(res_nullset, key == "FWER") %>%
    filter(! (Method == "ashq" & dist == "chisq4"),
           ! (grepl("scott", Method) & dist == "chisq4")) %>%
    ggplot(aes(x = Method, y = mean_value)) +
    geom_bar(color = "black", fill = "gray", stat = "identity") +
...
ggsave(file.path(outdir, "FigureSsim-null.pdf"), gp, width = 8, height = 3)
```
...
First, we look at the false discovery rate across the 100 replications.
We create the plot with a discrete color scale.

```{r, fig.width = 8, fig.height = 6}
fdr_bins <- c(0, .05, .07, .09, .11, .13, .15, .2, 1)
dplyr::filter(res_covset, key == "FDR") %>%
    dplyr::mutate(val = cut(mean_value, breaks = fdr_bins, include.lowest = TRUE)) %>%
    ggplot() +
    geom_tile(aes(x = setting, y = Method, fill = val)) +
    scale_x_discrete("Setting", expand = c(0, 0)) + 
    scale_y_discrete(expand = c(0, 0)) + 
    scale_fill_manual("FDR", values = RColorBrewer::brewer.pal(9, "PRGn")[2:9],
                      na.value = "gray50", drop = FALSE) +
    theme_bw() + 
    theme(axis.text.x = element_text(angle = 90, vjust = 1/2, hjust = 1)) +
    guides(fill = guide_legend(keywidth = 2, keyheight = 1)) +
    facet_grid(. ~ dist, scales = "free", space = "free", labeller= label_parsed) +
    ggtitle(paste0("Mean FDR across 100 replications (alpha = ", alpha, ")"))
```

We can create this plot with a continuous color scale as well.
However, it is incredibly hard to distinguish values if we look at the entire range of
possible values, so we will clamp them down to be between 0 and 0.2 (values above 0.2 will be
set to 0.2).

```{r, fig.width = 8, fig.height = 6}
gp_fdr <- dplyr::filter(res_covset, key == "FDR", setting != "null") %>%
    ggplot() +
    geom_tile(aes(x = setting, y = Method, fill = mean_value)) +
    scale_x_discrete("Setting", expand = c(0, 0)) + 
    scale_y_discrete(expand = c(0, 0)) + 
    scale_fill_gradient2("FDR", low = RColorBrewer::brewer.pal(6, "PRGn")[1],
                         high = RColorBrewer::brewer.pal(6, "PRGn")[6],
                         midpoint = alpha, na.value = "gray50", limits = c(0, .2),
                         oob = scales::squish) +
    theme_bw() +
    theme(axis.text.x = element_text(angle = 90, vjust = 1/2, hjust = 1)) +
    facet_grid(. ~ dist, scales = "free", space = "free", labeller = label_parsed) + 
...
We can also look at the true positive rates.

```{r, fig.width = 8, fig.height = 6}
gp_tpr <- dplyr::filter(res_covset, key == "TPR") %>%
    ggplot() +
    geom_tile(aes(x = setting, y = Method, fill = mean_value)) +
    scale_x_discrete("Setting", expand = c(0, 0)) + 
    scale_y_discrete(expand = c(0, 0)) + 
    scale_fill_distiller("TPR", palette = "Blues", limits = c(0, 1), direction = 1) +
    theme_bw() + 
    theme(axis.text.x = element_text(angle = 90, vjust = 1/2, hjust = 1)) +
    facet_grid(. ~ dist, scales = "free", space = "free", labeller = label_parsed) + 
    ggtitle(paste0("Mean TPR across 100 replications (alpha = ", alpha, ")"))
gp_tpr
```

Alternatively, rather than using heatmaps, we can use scatterplots to look at these values.
...
dplyr::select(res_covset, Method, setting, dist, key, mean_value, col, lty) %>%
    dplyr::filter(key %in% c("FDR", "TPR")) %>%
    tidyr::spread(key, mean_value) %>%
    ggplot(aes(x = FDR, y = TPR, color = Method)) +
    geom_point(alpha = 1/2) +
    scale_color_manual(values = col) +
    facet_grid(dist ~ setting, labeller = labeller(dist = label_parsed)) +
    theme_bw()
...
pfdr <- dplyr::select(res_covset, Method, setting, dist, key, mean_value) %>%
    ...
    ggtitle("Mean FDR for Informative and Noninformative Simulations") 
...
gp <- plot_grid(pfdr + coord_cartesian(c(0, .2), c(0, .2)) + guides(color = FALSE, linetype = FALSE),
...
pfdr <- dplyr::select(res_pi0set, Method, setting, pi0, key, mean_value, col, lty) %>%
    ...
    geom_point(alpha = 1/2, position = position_dodge(width = .01)) +
    scale_color_manual(values = col) +
...
pfdr + coord_cartesian(ylim = c(0, .25))
...
ggsave(file.path(outdir, "FigureSsim-unimodal.pdf"), uniplot, width = 8, height = 5)
```
