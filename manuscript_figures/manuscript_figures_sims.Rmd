...
```{r}
resmet <- dplyr::group_by(res, setting, dist, Method, key, performanceMetric, alpha)
...
resmet <- dplyr::mutate(resmet, signal = ifelse(grepl("varyinges", setting), dist, NA))
resmet <- dplyr::mutate(resmet, dist = ifelse(grepl("^varyinges$", setting), "gaussian", dist))
resmet <- dplyr::mutate(resmet, dist = ifelse(grepl("^varyinges-t$", setting), "t11", dist))
resmet <- dplyr::mutate(resmet, dist = ifelse(grepl("^varyinges-chisq$", setting), "chisq4", dist))
resmet <- dplyr::mutate(resmet, setting = ifelse(grepl("varyinges", setting), "varyinges", setting))
```
...
```{r, fig.width = 18, fig.height = 8}
res_noni <- dplyr::filter(res, grepl("informative-cubic", setting))
res_noni <- dplyr::group_by(res_noni, setting, dist, Method, key, performanceMetric, alpha)
res_noni <- dplyr::summarize(res_noni,
                             mean.uninfo = mean(value.uninfo, na.rm = TRUE),
                             se.uninfo = sd(value.uninfo, na.rm = TRUE) /
                                 sqrt(sum(!is.na(value.uninfo))))
res_noni <- dplyr::ungroup(res_noni)

noni_fdr <- dplyr::filter(res_noni, key == "FDR") %>%
    ggplot(aes(x = alpha, y = mean.uninfo, color = Method)) +
    geom_line(aes(linetype = Method), alpha = 0.85) +
    geom_errorbar(aes(ymin = mean.uninfo - se.uninfo, ymax = mean.uninfo + se.uninfo), width = 0.0025, alpha=0.5) +
    geom_abline(intercept = 0, slope = 1, lty = 2, color = "blue", alpha = 1/2) + 
    facet_grid(. ~ dist, scales = "free_x", labeller = label_parsed) +
    scale_linetype_manual(values = lty) + 
    scale_color_manual(values = col) +
...
res_pi0 <- dplyr::filter(resmet, setting == "varyingpi0-lowsignal",
                         alpha == 0.05)
res_pi0 <- dplyr::mutate(res_pi0,
                         mean.info = ifelse(is.nan(mean.info) & key == "FDR", 0, mean.info))
...
```{r, fig.width = 10, fig.height = 4}
res_info <- dplyr::filter(resmet, setting == "varyinginfo-lowsignal", alpha == 0.05)
