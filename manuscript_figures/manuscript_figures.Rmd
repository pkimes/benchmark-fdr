...
resfile <- objects[(grepl("yeast-", objects) | grepl("yeastH-", objects)) & 
                   grepl("de5", objects) & 
                  !grepl("uninf", objects)] 
labs <- rep("2000 Differentially expressed genes", 2)
labs[grepl("H", resfile)] <- "500 Differentially expressed genes"
excludeSet <- c("unadjusted", "bl-df02", "bl-df04", "bl-df05")
...
for (l in seq_along(resfile)){
  ...
  plist[[l]] <- plot_grid(p1, p2, ncol = 2, 
                   labels = c("", labs[l]), label_x = -0.65 )
}

# combine rows into one figure
Fig <- plot_grid(plotlist = plist, nrow = 2, 
