...
Fig4All <- plotMethodRanks(objects, colLabels = casestudy, alpha = alpha,
        ...
        theme(axis.text.x = element_text(angle = 90, vjust = 1/2, hjust = 1),
              legend.position = c(0.98, 0.93), legend.justification = c(1, 1),
...
ggdraw() +
  draw_plot(pfdr, 0.53, 0, 0.23, 1) +
  draw_plot(ptpr, 0.76, 0, 0.23, 1) +
...
ggsave(file.path(outdir, "Figure4.pdf"), width=9, height=5)
...
ggplot(rank.tab %>% filter(method != "bonf"), 
  ...
  theme_bw() +
  xlab("Proportion of maximum rejections in yeast simulation (n=5s)") +  
...
lineplot_tpr <- ggplot(dftab, aes(x = alpha, y = tpr_diff, color = method, linetype = method)) +
  ...
  gp
  ggsave(file.path(outdir, paste0("Figure_infCov_", org, ".pdf")), height = 3.5, width = 8)
}
```
