...
# strongly informative covariate
objects <- list.files( path, recursive=TRUE, pattern="rds", full.names=TRUE )
...
labs2 <- rep("Bimodal effect size", length(resfile))
labs2[grepl("II", resfile)] <- "Unimodal effect size"
excludeSet <- c("unadjusted", "bl-df02", "bl-df04", "bl-df05")
...
plot_grid(FigStrong, FigWeak, nrow = 2, 
          labels = paste0(LETTERS[1:2], "    "
                          c("Strongly informative covariate",
                            "Weakly informative covariate")))
ggsave(file.path(outdir, "Figure_yeast_varyingpi0.pdf"), width=10, height=8)
