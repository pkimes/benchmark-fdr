...
Fig4.rowOrder <- c("bonf", "bh", "ihw", "qvalue", "bl", "lfdr",
                   "ashq")
...
ggdraw() +
  ...
  draw_plot(Fig4All, 0, 0, 0.60, 1) +
  draw_plot_label(c("A", "B"), c(0, 0.57), c(1, 1), size = 15)
ggsave(file.path(outdir, "Figure4.pdf"), width=14, height=5)
...
Fig_impactofinfcov <- ggplot(dftab, aes(x = dataset, y = method, fill = diff_inf_percent)) + 
        ...
        scale_fill_gradientn("% Increase\nin rejections",
                             ...
                             guide = "colorbar", limits=c(-.05,0.99)) +
        theme_bw() +
        xlab("Case Study") +
        ylab("Method") +
