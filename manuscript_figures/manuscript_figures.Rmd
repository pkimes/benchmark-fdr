...
# calculate percentage diff as percentage of the smallest (nonzero) number 
# of rejections (i.e. using n rejects with informative or uninf for denominator,
# depending on which one was smaller while still being greater than zero)
dftab <- left_join(inftab, uninftab, by = "id") %>%
  dplyr::mutate(diff_inf = nrejects - uninf) %>%
  dplyr::mutate(basenum = ifelse(nrejects == 0 | uninf == 0, 
                                 pmax(nrejects, uninf),
                                 pmin(nrejects, uninf))) %>%
  dplyr::mutate(diff_inf_percent = diff_inf / basenum) %>%
...
Fig_impactofinfcov <- ggplot(dftab, aes(x = dataset, y = method, fill = diff_inf_percent)) + 
        ...
        theme(axis.text.x = element_text(angle = 90, vjust = 1/2, hjust = 1),
              legend.position = c(0.985, 0.93), legend.justification = c(1, 1),
              legend.background = element_rect(fill = scales::alpha("white", 3/4), color = 'black'),
              legend.direction = "horizontal",
              legend.text = element_text(size = 7),
              legend.title = element_text(size = 9),
