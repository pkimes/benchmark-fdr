...
Fig_impactofinfcov_fc <- ggplot(dftab, aes(x = dataset, y = method, fill = diff_inf_ratio)) + 
        geom_tile() + 
...
Fig_impactofinfcov_fc <- ggplot(dftab, aes(x = casestudy, y = method, fill = diff_inf_ratio)) + 
        geom_tile() + 
        scale_fill_gradientn("Fold Change\n(log2)",
                             colors = c("darkblue","white","darkred"), 
                             limits = c(-0.5, topRange), 
...
ggdraw() +
  draw_plot(pfdr, 0.53, 0.025, 0.24, 0.93) +
  draw_plot(ptpr, 0.77, 0.025, 0.24, 0.93) +
  draw_plot(Fig_impactofinfcov_fc, 0, 0, 0.52, 1) +
  draw_plot_label(c("A", "B"), c(0, 0.53), c(1, 1), size = 15) +
  draw_plot_label("Simulation", 0.71, 0.06, size =11, fontface = "plain")
ggsave(file.path(outdir, "Figure_impactofinfcov_avg_sim_fc.pdf"), width=9.5, height=5)
