...
# Informativeness of covariates in Yeast simulation 
...
dftab <- left_join(inftab, uninftab, by = "id") %>%
  ...
  summarize(tpr_diff = mean(tpr_diff),
...
plot_fun <- function(var){
  ...
  plot  <- ggplot(dftab %>% filter(!is.na(get(var))), 
    ...
    scale_fill_distiller(paste0("Change\nin ", toupper(var)),
    ...
    ggtitle(paste0("Impact of informative covariate on ", toupper(var))) +
    theme(axis.text.x = element_text(angle = 90, vjust = 1/2, hjust = 1),
