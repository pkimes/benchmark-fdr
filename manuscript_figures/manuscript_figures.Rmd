...
for (org in c("yeast-", "polyester")){
  ...
  for (l in seq_along(resfile)){
    ...
    tmp3 <- tidy_df(x, colLabels = rep(filename[l], length(x)), 
                    fill =  "TNR", annotate = NULL, 
                    alpha = aseq) %>%
      mutate(tnr = nrejects) %>%
      select(method, casestudy, replicate, tnr, alpha)
    ...
    if (type[l] != "null"){
      ...
      tmp <- left_join(tmp1, tmp2, by = c("method", "casestudy", "replicate", "alpha")) %>%
        left_join(tmp3, by = c("method", "casestudy", "replicate", "alpha")) %>%
        mutate(inform = inform[l],
               type = type[l],
               size = size[l])
    }else{
      tmp <- left_join(tmp2, tmp3, by = c("method", "casestudy", "replicate", "alpha")) %>%
        mutate(inform = inform[l],
               type = type[l],
               size = size[l])
    }
...
lineplot_fdr <- ggplot(dftab,
                       aes(x = alpha, y = fdr_diff, color = method, linetype = method)) +
    ...
    scale_y_continuous(expression(paste("Mean ", Delta, "FDR (informative-uninformative)")), 
                       breaks = seq(-1, 1, .005), labels = scales::percent) + 
    ggtitle("FDR difference with informative covariate") +
    theme_classic() +
    theme(axis.title = element_text(face="bold", size = 10),
          plot.title = element_text(face="bold", size = 11, hjust = 0.8))
  
lineplot_tpr <- ggplot(dftab, aes(x = alpha, y = tpr_diff, color = method, linetype = method)) +
    geom_hline(yintercept = 0, lty = 2, color = "blue", alpha = 1/2) + 
    ...
    scale_color_manual(values = col) +
    scale_x_continuous("alpha", breaks = seq(0, 1, .01)) + 
    scale_y_continuous(expression(paste("Mean ", Delta, "TPR (informative-uninformative)")), 
                       labels = scales::percent) + 
    ggtitle("TPR difference with informative covariate") +
    theme_classic() +
    theme(axis.title = element_text(face="bold", size = 10),
          plot.title = element_text(face="bold", size = 11))
  
  gp <- plot_grid(lineplot_fdr + guides(color = FALSE, linetype = FALSE),
                  lineplot_tpr + guides(color = FALSE, linetype = FALSE),
                  labels = LETTERS[1:2], nrow = 1)
  gp <- plot_grid(gp, get_legend(lineplot_fdr), rel_widths = c(1, 1/8))
  gp
  ggsave(file.path(outdir, paste0("Figure_infCov_", gsub("-", "", org), ".pdf")), 
                   height = 3.5, width = 8)
}
```
...
# log-scale color log2 ratio fill
topRange <- max(dftab$diff_inf_ratio, na.rm = TRUE)
botRange <- min(dftab$diff_inf_ratio, na.rm = TRUE)

Fig_impactofinfcov <- ggplot(dftab, aes(x = dataset, y = method, fill = diff_inf_ratio)) + 
        geom_tile() + 
        geom_text(aes(label = ifelse(is.na(topR), "", #adaptive text lab color
                                     ifelse(topR > 1, signif(topR,2), ""))), 
                  color = "white", size = 2) +
        geom_text(aes(label = ifelse(is.na(topR), "", 
                                     ifelse(topR <= 1, signif(topR,2), ""))), 
                  color = "black", size = 2)+
        scale_fill_gradientn("Fold Change\n(log2)",
                             colors = c("darkblue","white","darkred"), 
                             limits = c(-0.5, topRange), 
                             values = scales::rescale(c(-0.5, 0, topRange))) +
        theme_bw() +
        xlab("Case Study") +
        ylab("Method") +
        facet_grid( ~ casestudy + cov, scales = "free", space = "free") +
        ...
        theme(axis.text.x = element_text(angle = 90, vjust = 1/2, hjust = 1),
              ...
              legend.background = element_rect(fill = scales::alpha("white", 3/4), color = 'black'),
              legend.direction = "horizontal",
              legend.text = element_text(size = 7, angle=45, margin = margin(t=0)),
              legend.title = element_text(size = 9, margin=margin(t=-10)),
              legend.margin = margin(t=2, r=10, b=-2, l=10))

Fig_impactofinfcov
ggsave(file.path(outdir, "Figure_impactofinfcov_fc.pdf"), width = 15.5, height = 4.25)
...
dftab <- left_join(dftab, maxVals, by = c("casestudy")) %>%
  ...
  mutate(topR = ifelse(diff_inf_ratio == topR, diff_inf_ratio, NA)) %>%
  ungroup() %>%
  mutate(casestudy = factor(casestudy, levels = colOrder)) %>%
  mutate(casestudyN = paste0(casestudy, "(", maxd, ")")) %>%
  mutate(casestudyN = factor(casestudyN, 
...
Fig_impactofinfcov <- ggplot(dftab, aes(x = casestudy, y = method, fill = diff_inf_percent*100)) + 
        ...
        xlab("Case Study") +
        ylab("Method") +
        ggtitle("Impact of informative covariate") +
        theme(axis.text.x = element_text(angle = 90, vjust = 1/2, hjust = 1)) +
        geom_text(aes(label = ifelse(is.na(top), "", #adaptive text lab color
                                     ifelse(top > 0.7, scales::percent(top), ""))), 
                  color = "white", size = 2) +
...
Fig_impactofinfcov <- ggplot(dftab, aes(x = casestudy, y = method, fill = diff_inf_ratio)) + 
        geom_tile() + 
        scale_fill_gradientn("Fold Change\n(log2)",
                             colors = c("darkblue","white","darkred"), 
                             limits = c(-0.5, topRange), 
...
Fig_impactofinfcov
ggsave(file.path(outdir, "Figure_impactofinfcov_avg_fc.pdf"), width = 5, height = 4)
...
ggdraw() +
  draw_plot(pfdr, 0.53, 0.025, 0.24, 0.93) +
  draw_plot(ptpr, 0.77, 0.025, 0.24, 0.93) +
...
objects <- list.files( path, recursive=TRUE, pattern="rds", full.names=TRUE )
resfile <- objects[(grepl("yeastU-", objects) | grepl("yeastHU-", objects) | 
                    grepl("yeastIIU-", objects) | grepl("yeastIIHU-", objects)) & 
                   grepl("de5", objects) & 
                  !grepl("uninf", objects)] 
labs <- rep("30% Non-null", length(resfile))
labs[grepl("H", resfile)] <- "7.5% Non-null"
