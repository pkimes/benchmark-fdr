...
Fig4.rowOrder <- levels(plotMethodRanks(objects, colLabels = casestudy, alpha = 0.10,
                            tableOnly = TRUE)$method)
Fig4 <- plotMethodRanks(objects, colLabels = casestudy, alpha = 0.10)
Fig4
ggsave(file.path(outdir, "Figure4a.pdf"), width=6, height=4)

Fig4All.rowOrder <- levels(plotMethodRanks(objects, colLabels = casestudy, alpha = 0.10, 
                               tableOnly = TRUE, excludeMethods = NULL)$method)
Fig4All <- plotMethodRanks(objects, colLabels = casestudy, alpha = 0.10,
                        excludeMethods = NULL)
Fig4All
ggsave(file.path(outdir, "Figure4a_all.pdf"), width=6, height=4)
...
library(xtable)
tabS3 <- plotMethodRanks(objects, colLabels = casestudy, alpha = 0.10,
                        tableOnly = TRUE)
tabS3 <- tabS3 %>% 
...
labs <- unlist(lapply(str_split(dataset[wcs], pattern = "-benchmark.rds"), 
                      function(x) x[1]))
plotMethodRanks(objs, colLabels = labs, alpha = 0.10, xlab = "Dataset", Nlabel = FALSE) + 
  ggtitle("GSEA")
...
# by dataset
pa <- plotMethodRanks(objs, colLabels = ds, alpha = 0.10, xlab = "Dataset") + 
  ggtitle("scRNA-seq")

# by method
pb <- plotMethodRanks(objs, colLabels = meth, alpha = 0.10, xlab = "Method") + 
  ggtitle("scRNA-seq")

# by covariate
pc <- plotMethodRanks(objs, colLabels = cv, alpha = 0.10, xlab = "Covariate") + 
  ggtitle("scRNA-seq")
