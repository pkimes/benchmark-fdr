...
# point to where yeast results are (within 'path' defined above)
resdir <- file.path(path, "RNAseq/yeast-results")
...
# Figure 2E - upsetr plot -- this solution currently chops off left margin hist
...
Fig2
ggsave(file.path(outdir, "Figure2.pdf"), width=10, height=12)
```
...
case_dat <- ranks_avg %>%
  group_by( casestudy) %>%
  summarize( nmethods = n()) 

method_dat <- ranks_avg %>%
  group_by( method) %>%
  summarize( meta_rank = mean(mean_rank))
...
Fig4alt <- Fig4 +
  ...
  theme(panel.background = element_rect(colour="grey60", fill="grey60",
                                        size = 0.5),
...
Fig4b <- ggplot( ranks_avg, aes(x=method, y=mean_rank, group=casestudy, col=casestudy) ) +
  geom_line() + geom_point() +
  labs(y="Mean rank", x="", col="Case study") +
