...
pc <- strat_hist(res, pvalue = "pval",
           covariate = "ind_covariate", maxy = 7, axislabs = FALSE) +
...
cov.methods <- c("ihw", "bl", "lfdr", "fdrreg-e", "fdrreg-t")
...
uninftab <- dftab %>% 
  ...
  #dplyr::rename(nrejects = "uninf")

inftab <- dftab %>%
  dplyr::filter(inform == "inf") %>%
  dplyr::select(-inform)

rowOrder <- c("ihw", "bl", "lfdr", "fdrreg-e", "fdrreg-t")

dftab <- left_join(inftab, uninftab, by = "id") %>%
  dplyr::mutate(diff_inf = nrejects - uninf) %>%
  dplyr::mutate(diff_inf_percent = diff_inf / nrejects) %>%
...
dftab <- dftab %>%
  ...
  summarize(diff_inf_percent = mean(diff_inf_percent, na.rm = TRUE))
...
Fig_impactofinfcov <- ggplot(dftab, aes(x = casestudy, y = method, fill = diff_inf_percent)) + 
        geom_tile() + 
        scale_fill_gradientn("% Increase\nin rejections",
                             labels = scales::percent, trans="log1p",
                             colours = c("darkblue","white","darkred"), 
...
Fig_impactofinfcov
ggsave(file.path(outdir, "Figure_impactofinfcov_avg.pdf"), width = 5.75, height = 4)
```


# Informativeness of covariates in Yeast & Polyester simulation 

As an alternative to the line plot above (Figure S7), we make a heatmap plot that
displays the mean difference within simulation replicate.

```{r}

for (org in c("yeast", "polyester")){
...
for (l in seq_along(resfile)){
  ...
  tmp2 <- tidy_df(x, colLabels = rep(filename[l], length(x)), 
                 fill =  "FDR", annotate = NULL, alpha = 0.10) %>%
    mutate(fdr = nrejects) %>%
    select(method, casestudy, replicate, fdr)
  tmp3 <- tidy_df(x, colLabels = rep(filename[l], length(x)), 
                  fill =  "TNR", annotate = NULL, alpha = 0.10) %>%
    mutate(tnr = nrejects) %>%
  select(method, casestudy, replicate, tnr)
  
  if (type[l] != "null"){
    tmp1 <- tidy_df(x, colLabels = rep(filename[l], length(x)), 
                    fill =  "TPR", annotate = NULL, alpha = 0.10) %>%
      mutate(tpr = nrejects) %>%
    select(method, casestudy, replicate, tpr)

    tmp <- left_join(tmp1, tmp2, by = c("method", "casestudy", "replicate")) %>%
      left_join(tmp3, by = c("method", "casestudy", "replicate")) %>%
      mutate(inform = inform[l],
             type = type[l],
             size = size[l])
  }else{
    tmp <- left_join(tmp2, tmp3, by = c("method", "casestudy", "replicate")) %>%
      mutate(inform = inform[l],
             type = type[l],
             size = size[l])
  }
...
# take difference within replicate, then average differences across
# type and size and method

dftab <- tab %>%
  filter(method %in% cov.methods) %>%
  dplyr::mutate(id = paste(type, size, method, replicate, sep = "-"))

uninftab <- dftab %>% 
  filter(inform == "uninf") %>%
  select(id, fdr, tpr, tnr) %>%
  rename(fdr = "fdr_uninf",
         tnr = "tnr_uninf",
         tpr = "tpr_uninf")
...
dftab <- left_join(inftab, uninftab, by = "id") %>%
  mutate(tpr_diff = tpr-tpr_uninf,
         fdr_diff = fdr-fdr_uninf,
         tnr_diff = tnr-tnr_uninf) %>%
  group_by(type, size, method) %>%
  summarize(tpr_se = sqrt(sd(tpr_diff)/n()),
            fdr_se = sqrt(sd(fdr_diff)/n()),
            tnr_se = sqrt(sd(tnr_diff)/n()),
            tpr_diff = mean(tpr_diff),
            fdr_diff = mean(fdr_diff),
            tnr_diff = mean(tnr_diff)) %>%
  mutate(method = factor(method, levels = rowOrder)) %>%
  mutate(fdr_diff = ifelse(type == "null", NA, fdr_diff),
         tpr_diff = ifelse(type == "null", NA, tpr_diff),
         fdr_se = ifelse(type == "null", NA, fdr_se),
         tpr_se = ifelse(type == "null", NA, tpr_se))

plot_fun <- function(var){
  
  plot  <- ggplot(dftab %>% filter(!is.na(get(var))), 
                  aes(x = interaction(type, size), 
                                        y = method)) + 
    geom_tile(aes(fill=get(var))) + 
    scale_fill_gradient2(paste0("Change\nin ", toupper(gsub("_diff", "", var))),
                         low = "red", midpoint = 0, mid = "white", high = "blue",
                         labels = scales::percent) +
    theme_bw() +
    xlab("Experiment") +
    ylab("Method") +
    facet_grid( ~ type + size, scales = "free", space = "free") +
    ggtitle(paste0("Impact of informative covariate on ", toupper(gsub("_diff", "", var)))) +
    theme(axis.text.x = element_text(angle = 90, vjust = 1/2, hjust = 1)) +
    geom_text(aes(label = scales::percent(get(var))),
            color = "black", size = 3)
  
  return(plot)
}

# arrange in multipanel figure?
plot_fun("tpr_diff")
ggsave(file.path(outdir, paste0("Figure_infCov_tpr_", org, ".pdf")), height = 5, width = 5)

plot_fun("fdr_diff")
ggsave(file.path(outdir, paste0("Figure_infCov_fpr_", org, ".pdf")), height = 5, width = 5)

plot_fun("tnr_diff")
ggsave(file.path(outdir, paste0("Figure_infCov_tnr_", org, ".pdf")), height = 5, width = 7)
