...
bind_rows(Sims = consistency2_sims, CS = consistency2_data, .id = "analysis") %>%
    dplyr::mutate(Method = reorder(Method, propEither95)) %>%
    ggplot(aes(x = Method, y = propEither95, color = Method, group = analysis)) +
    geom_line(aes(linetype = analysis), color = 'black', alpha = 1/2) +
    geom_point() + 
    scale_color_manual(values = col, guide = FALSE) +
    scale_x_discrete(NULL) + 
    scale_y_continuous("Percent of settings w/ TPR or #rejections\n <95% of BH/q-value", labels = scales::percent) + 
    theme_classic() +
    theme(axis.title = element_text(face = "bold", size = 10),
          plot.title = element_text(face = "bold", size = 14)) +
...
p1c <- bind_rows(Sims = consistency2_sims, CS = consistency2_data, .id = "Analysis") %>%
    dplyr::mutate(Method = reorder(Method, propEither95),
                  ...
                  Aanlysis = factor(Analysis, levels = levels(Analysis)[c(2, 3, 1)])) %>%
    ggplot(aes(x = Method, y = propEither95, color = Method, group = Analysis)) +
    geom_line(aes(linetype = Analysis), color = 'black', alpha = 1/2) +
    geom_point(size = 3) + 
    scale_color_manual(values = col, guide = FALSE) +
    scale_x_discrete(NULL) + 
    scale_y_continuous("% settings w/ metric\n < 95% of BH and q-value", labels = scales::percent) + 
    theme_classic() +
    theme(axis.title = element_text(face = "bold", size = 10),
          plot.title = element_text(face = "bold", size = 14)) +
    ggtitle("Consistency (I)")
...
p1d <- bind_rows(Sims = consistency_sims, CS = consistency_data, .id = "Analysis") %>%
    ...
    scale_color_manual(values = col, guide = FALSE) +
    scale_x_discrete(NULL) + 
    scale_y_continuous("SD of log-ratio relative to\nmean of BH and q-value") + 
    expand_limits(y = 1) +
    theme_classic() +
    theme(axis.title = element_text(face = "bold", size = 10),
          plot.title = element_text(face = "bold", size = 14)) +
    ggtitle("Consistency (II)")
...
FigS <- plot_grid(p1title, p1ab, p1cd, ncol = 1, rel_heights = c(.15, 1, 1))
FigS
