...
bind_rows(yeast = fdrcontrol_yeast, sims = fdrcontrol_sims, .id = "analysis") %>%
    ggplot(aes(x = analysis, y = pExceeds, color = Method, group = Method)) +
    geom_line(aes(linetype = Method), alpha = 1/2, position = position_dodge(width = 1/4)) +
    geom_point(position = position_dodge(width = 1/4)) + 
    scale_linetype_manual(values = lty) + 
    scale_color_manual(values = col) +
    scale_x_discrete(NULL) + 
    scale_y_continuous("Percent Settings Exceeding nominal FDR", labels = scales::percent) + 
    expand_limits(y = 0) +
    theme_classic() +
