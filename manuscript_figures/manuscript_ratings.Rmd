...
##saveRDS(datres, "datres-table.rds")
```

## Simulations

We can use the results stored in `resmet` for simulation settings.
For easier plotting, we replace create a shorter distribution string column.

```{r}
resmet <- dplyr::mutate(resmet, sdist = factor(dist, sort(levels(dist)), c("chisq", "normal", "t11", "t5")))
```

We only look at results at the desired alpha cutoff, and resmet `fdrreg-e` from this comparison.

```{r}
resmet <- dplyr::filter(resmet, Method != "fdrreg-e", alpha == alpha.thresh)
```

We also rank each method in each setting according to FDR and TPR.

```{r}
resmet <- dplyr::group_by(resmet, setting, dist, inform, pi0, signal, ntests, sdist, key)
resmet <- dplyr::mutate(resmet, rank = rank(-mean.info))
...
```{r}
resmet <- dplyr::mutate(resmet, setting = ifelse(setting == "varyingpi0-t", "varyingpi0", setting))
```
...
fdrcontrol_yeast <- yeast %>%
    ...
    dplyr::summarize(nExceeds = sum((mean - se) > alpha.thresh),
                     pExceeds = round(nExceeds / n() * 100, 2)) %>%
    dplyr::ungroup()
dplyr::arrange(fdrcontrol_yeast, pExceeds)
...
power_sims <- resmet %>%
    ...
    dplyr::ungroup()
dplyr::arrange(power_sims, meanRank)
...
power_yeast <- yeast %>%
    ...
    dplyr::summarize(meanRank = mean(rank),
                     medianRank = median(rank)) %>%
    dplyr::ungroup()
dplyr::arrange(power_yeast, meanRank)
...
```{r}
## robustness as variance of FDR/TPR as % increase vs BH/qvalue
robustness_sims <- resmet %>%
    ...
    dplyr::ungroup()
dplyr::arrange(robustness_sims, key, -cvGain)
...
robustness_yeast <- yeast %>%
    ...
    dplyr::summarize(meanGain = mean(mean / (bh + qvalue) * 2),
                     ...
                     cvGain = sdGain / meanGain) %>%
    dplyr::ungroup()
dplyr::arrange(robustness_yeast, key, -cvGain)
...
dplyr::arrange(robustness_data, -cvGain)
