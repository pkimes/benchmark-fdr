...
rejection_scatter_bins( sGSEA, "ind_covariate", threshold=15, bins=4, as_fraction=FALSE, supplementary=FALSE)
```
...
getOverlapIndexes <- function( labs=c("lfdr", "ihw-a10"), alpha=0.1 ){
  rowSums( assays(sGSEA)[["qvalue"]][,labs] < alpha) == length(labs) &
  !(rowSums( assays(sGSEA)[["qvalue"]][,c("lfdr","ihw-a10","bl-df03","qvalue","bh", "bonf")] < alpha, na.rm=TRUE ) > (length(labs)))
}

set1 <- getOverlapIndexes( c("lfdr", "ihw-a10"), alpha=0.1 )
set2 <- getOverlapIndexes( c("lfdr","ihw-a10","bl-df03","qvalue","bh"), alpha=0.1 )
set3 <- getOverlapIndexes( c("lfdr","ihw-a10","bl-df03","qvalue","bh", "bonf"), alpha=0.1 )
...
Plotting the methods overlap

```{r}

sGSEASub <- sGSEA[,!apply( is.na( assays(sGSEA)[["qvalue"]] ), 2, all )]
sGSEASub <- sGSEASub[,-which(grepl("ihw", colData( sGSEASub )$blabel) & colData( sGSEASub )$param.alpha != 0.1)]
sGSEASub <- sGSEASub[,-which( grepl("bl", colData( sGSEASub)$blabel) & 
                   colData(sGSEASub)$param.smooth.df != 2L )]
sGSEASub <- sGSEASub[,!colData(sGSEASub)$blabel %in% "unadjusted"]

plotMethodsOverlap(sGSEASub, order.by = "freq", nsets=10)
