---
title: "Simulation Study: Informative Covariate (cosine)"
...
gauss_file <- file.path(resdir, "informative-cosine-benchmark-gaussian.rds")
tdist_file <- file.path(resdir, "informative-cosine-benchmark-t5.rds")
tdist11_file <- file.path(resdir, "informative-cosine-benchmark-t11.rds")
chisq_file <- file.path(resdir, "informative-cosine-benchmark-chisq4.rds")
...
resdfiu <- dplyr::full_join(select(resdf, rep, blabel, param.alpha, key, performanceMetric, alpha, value),
                            select(resdfu, rep, blabel, param.alpha, key, performanceMetric, alpha, value),
...
```{r t5-parameters}
tstat <- rnorm_generator(6)    # functional: dist of alternative test stats
...
```{r t11-parameters}
tstat <- rnorm_generator(3)     # functional: dist of alternative test stats
tstat_dist <- rt_perturber(11)  # functional: sampling dist/noise for test stats
...
```{r chisq4-parameters}
tstat <- rnorm_generator(15)       # functional: dist of alternative test stats
...
onerun <- simIteration(bdchi, m = m, pi0 = pi0, tstat = tstat, tstat_dist = tstat_dist,
                       icovariate = icovariate, null_dist = null_dist, execute = FALSE)
```
