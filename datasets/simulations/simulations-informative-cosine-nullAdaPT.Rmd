---
title: "Simulation Study: AdaPT with null model option (cosine covariate)" 
...
again with Gaussian, t-distributed, and Chi-Squared distributed noise.

This set of simulations differs from `simulations-informative-cosine.Rmd` only
in the implementation of the AdaPT method for multiple testing correction. This
is a follow up analysis similar to that described and performed in
`simulations-informative-step-nullAdaPT.Rmd`. For more details on the parameter settings
of the two additional AdaPT methods included in the comparison, please check the
other `-nullAdaPT.Rmd` analysis. 

The analysis in `simulations-informative-step-nullAdaPT.Rmd` was carried out
to determine whether the additional `adapt-withnull` and `adapt-onlynull` methods
could provide an improvement over the `adapt-glm` parameter setting in the case
of low informativeness in the independent covariate. This analysis aims to examine
how the `adapt-withnull` and `adalpt-onlynull` parameter settings perform in
comparison to the default `adapt-glm` settings when the covariate is more
informative.
...
gauss_file <- file.path(resdir, "nullAdaPT-informative-cosine-benchmark-gaussian.rds")
tdist_file <- file.path(resdir, "nullAdaPT-informative-cosine-benchmark-t5.rds")
tdist11_file <- file.path(resdir, "nullAdaPT-informative-cosine-benchmark-t11.rds")
chisq_file <- file.path(resdir, "nullAdaPT-informative-cosine-benchmark-chisq4.rds")
...
pi0 <- pi0_cosine(0.90)           # numeric: proportion of null hypotheses
...
First, we consider the setting with Gaussian noise.
...
Next, we consider the setting with t-distributed noise.
...
Finally, we consider the setting with chi-squared distributed noise.
...
For the chi-squared distributed noise setting, we must change the "mode" setting for ASH
