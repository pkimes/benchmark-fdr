---
title: "Simulation Summary Figures (simulations)"
author: "Rafalab"
output: 
    html_document:
        toc: true
...
for (ia in c(1:10, 20, 30, 40, 50)) {
    summary_file <- file.path("results", paste0("summary_alpha", ia, ".rds"))
...
for (ia in seq_len(length(alphas))) {
    ...
    ires <- readRDS(file.path("results", paste0("summary_alpha", alphas[ia], ".rds")))
...
```{r, fig.width = 18, fig.height = 8}
res_noise <- dplyr::filter(resmet, setting == "informative-cosine")
...
res_pi0 <- dplyr::filter(resmet, setting == "varyingpi0-lowsignal", alpha == 0.05)
...
being null is one value if the covariate is less than 0.8, and another value if the covariate
is above 0.8. The "informativeness" is measured by the difference between these two probabilities,
with informativeness 0 and 1 corresponding to no difference and maximal difference (i.e. 0 and 1),
respectively. We plot the false discovery proportion and true positive rate at a nominal alpha
...
```{r, fig.width = 18, fig.height = 8}
res_ua <- dplyr::filter(resmet, setting == "varyinges", grepl("N\\(0, 1\\)", dist))
...
```{r, fig.width = 18, fig.height = 8}
res_ua <- dplyr::filter(resmet, setting == "varyinges", grepl("t\\[11\\]", dist))
