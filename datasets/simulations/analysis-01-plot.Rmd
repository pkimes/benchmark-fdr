...
<!-- ######################################################################## -->
## Plot Cross-Setting Metrics (100 replications) just "alpha = 0.05"
<!-- ######################################################################## -->

In the previous section, we generated a single plot for each metric under each simulation setting as a function of the nominal FDR threshold over a range of 0.01 to 0.10. In this section, we instead just focus on the FDR threshold of 0.05 and generate plots as a function of the simulation settings. Thus, each family of simulation settings is reduced to a single plot for each metric, making it easier to summarize and compare the performance of each method across a large set of settings.

Here, we simply parse and save the merged performance metrics. Results are plotted in `analysis-03-plot.Rmd`.
...
for (dd in dl) {
    ...
    for (f in fl) {
        ...
        print(paste0("done with ", f, "; time: ", proc.time()[3] - t1[3]))
    }
    tsbc <- bind_rows(tsbl, .id = "param") %>% as.tibble

    ## save the parsed tables for easier future access
