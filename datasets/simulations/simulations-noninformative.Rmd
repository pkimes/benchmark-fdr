---
title: "Simulation Study: Non-Informative Covariate"
...
tests with only an non-informative covariate. We include simulation results
...
gauss_file <- file.path(resdir, "noninformative-benchmark-gaussian.rds")
tdist_file <- file.path(resdir, "noninformative-benchmark-t5.rds")
tdist11_file <- file.path(resdir, "noninformative-benchmark-t11.rds")
chisq_file <- file.path(resdir, "noninformative-benchmark-chisq4.rds")
...
pi0 <- 0.90                       # numeric: proportion of null hypotheses
...
covariateLinePlot(lapply(res, function(x) { x[, -which(colnames(x) == "ashq")] }),
...
if (numberMethodsReject(resdf, alphacutoff = ualpha, filterSet = c(excludeSet, "ashq")) >= 3) {
    aggupset(lapply(res, function(x) { x[, -which(colnames(x) == "ashq")] }),
