...
```{r gauss-parameters}
tstat <- rnorm_generator(4)       # functional: dist of alternative test stats
...
Since both `scott-empirical` and `scott-theoretical` are not appropriate when
the test statistics are not approximately normal, we also plot the results without
these methods.

```{r chisq4-metrics-averages-nofdrreg, results = "hide"}
plotsim_average(resdf, met = "rejections",
                filter_set = c(excludeSet, "scott-empirical", "scott-theoretical"),
                merge_ihw = TRUE, errorBars = TRUE) 
