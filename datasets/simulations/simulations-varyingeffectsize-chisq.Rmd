...
```{r spiky-parameters}
tstat <- function(n) { abs(sampler_spiky(n)) } # functional: dist of alternative test stats
seed <- 778
```
...
tstat <- function(n) { abs(sampler_flat_top(n)) }       # functional: dist of alternative test stats
seed <- 980
```
...
```{r skew-parameters}
tstat <- function(n) { abs(sampler_skew(n)) }       # functional: dist of alternative test stats
seed <- 206
```
...
tstat <- function(n) { abs(sampler_bimodal(n)) }       # functional: dist of alternative test stats
seed <- 913
```
