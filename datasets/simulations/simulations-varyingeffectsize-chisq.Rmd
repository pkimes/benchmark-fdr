---
title: "Simulation Study: Varying Effect Size (chisq-4)"
...
presented in `simulations-varyingeffectsize.Rmd`, however, Chi-Squared
distributed test statistics are used. Additionally, since the effect
must be non-negative (to be a valid value for the non-centrality parameter
of the a Chi-Squared distribution), the absolute value of all effect sizes
are used. Note that this modification still maintains a unimodal distribution
of the effect sizes for all settings except the "bimodal" case below.
...
spiky_file <- file.path(resdir, "varyinges-chisq-benchmark-spiky.rds")
flattop_file <- file.path(resdir, "varyinges-chisq-benchmark-flattop.rds")
skew_file <- file.path(resdir, "varyinges-chisq-benchmark-skew.rds")
bimodal_file <- file.path(resdir, "varyinges-chisq-benchmark-bimodal.rds")
...
```{r}
bd <- modifyBMethod(bd, "ashq", mode = "empirical")
```
...
```{r spiky-parameters}
es_dist <- function(n) { abs(sampler_spiky(n)) } # functional: dist of alternative test stats
seed <- 778
```
...
onerun <- simIteration(bd, m = m, pi0 = pi0, es_dist = es_dist, ts_dist = ts_dist,
...
es_dist <- function(n) { abs(sampler_flat_top(n)) }       # functional: dist of alternative test stats
seed <- 980
```
...
```{r skew-parameters}
es_dist <- function(n) { abs(sampler_skew(n)) }       # functional: dist of alternative test stats
seed <- 206
```
...
es_dist <- function(n) { abs(sampler_bimodal(n)) }       # functional: dist of alternative test stats
seed <- 913
```
