...
re_blabel <- function(tab) {
    tsb %>%
        mutate(blabel = fct_recode(f = blabel, "IHW" = "ihw-a05",
               ...
               takes_cov = blabel %in%
                   c("IHW", "Boca-Leek", "FDRreg (theoretical)", "FDRreg (empirical)")) %>%
...
```{r}
## older run with alternative tests at 1.5.
f <- "data-psim/M100/_pi0-uniform-delta15/results-performanceMetrics-alpha05.rds"
tsb <- readRDS(f)
