...
resfile_n5_uninfCov <- file.path(resdir, "polyester-results-null5-uninfCov.rds")
resfile_n10_uninfCov <- file.path(resdir, "polyester-results-null10-uninfCov.rds")
...
We use the [polyester](https://bioconductor.org/packages/release/bioc/html/polyester.html) Bioconductor package to simulate RNA-Seq reads. The package requires a count matrix as input which estimates the mean variance relationship. Here we start with the yeast data with 48 biological replicates in each of 
...
simulateOneSplit <- function(X, rseed, 
  ...
  gs <- sample(1:nrow(counts), ngenes, replace = TRUE)
