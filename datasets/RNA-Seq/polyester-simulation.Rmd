...
# devtools::install_github("areyesq89/SummarizedBenchmark", ref="fdrbenchmark") # needed b/c BioC devel branch is 
library(SummarizedBenchmark)

# load helper functions
sourceDirectory(file.path("../R/"))
...
if (!file.exists(file.path("polyester-results-null5.RDS"))){
  null5 <- bplapply(X=1:B, FUN=simulateOneSplit, counts=counts,
                   paramPolyester=params, nDE=nDE, sampleSize=sampleSize, bd=bd,
                   BPPARAM=multicoreParam)
  saveRDS(null5, file=file.path("polyester-results-null5.RDS"))
} else {
  null5 <- readRDS(file=file.path("polyester-results-null5.RDS"))
}
```
...
if (!file.exists(file.path("polyester-results-null10.RDS"))){
  null10 <- bplapply(X=1:B, FUN=simulateOneSplit, counts=counts,
                   paramPolyester=params, nDE=nDE, sampleSize=sampleSize, bd=bd,
                   BPPARAM=multicoreParam)
  saveRDS(null10, file=file.path("polyester-results-null10.RDS"))
} else {
  null10 <- readRDS(file=file.path("polyester-results-null10.RDS"))
}
```
...
if (!file.exists(file.path("polyester-results-de5.RDS"))){
  de5 <- bplapply(X=1:B, FUN=simulateOneSplit, counts=counts,
                   paramPolyester=params, nDE=nDE, sampleSize=sampleSize, bd=bd,
                   BPPARAM=multicoreParam)
  saveRDS(de5, file=file.path("polyester-results-de5.RDS"))
} else {
  de5 <- readRDS(file=file.path("polyester-results-de5.RDS"))
}
```
...
if (!file.exists(file.path("polyester-results-de10.RDS"))){
  de10 <- bplapply(X=1:B, FUN=simulateOneSplit, counts=counts,
                   paramPolyester=params, nDE=nDE, sampleSize=sampleSize, bd=bd,
                   BPPARAM=multicoreParam)
  saveRDS(de10, file=file.path("polyester-results-de10.RDS"))
} else {
  de10 <- readRDS(file=file.path("polyester-results-de10.RDS"))
}
```
