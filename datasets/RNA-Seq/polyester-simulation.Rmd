...
resfile_n5 <- file.path(resdir, "polyesterY-results-null5.rds")
resfile_n10 <- file.path(resdir, "polyesterY-results-null10.rds")
resfile_d5 <- file.path(resdir, "polyesterY-results-de5.rds")
resfile_d10 <- file.path(resdir, "polyesterY-results-de10.rds")

resfile_n5_uninfCov <- file.path(resdir, "polyesterY-results-null5-uninfCov.rds")
resfile_n10_uninfCov <- file.path(resdir, "polyesterY-results-null10-uninfCov.rds")
resfile_d5_uninfCov <- file.path(resdir, "polyesterY-results-de5-uninfCov.rds")
resfile_d10_uninfCov <- file.path(resdir, "polyesterY-results-de10-uninfCov.rds")
...
Try using the yeast data instead - want all samples of the same condition, and the
zebrafish dataset only has 3v3.
...
if (!file.exists(resfile_n5)){
  null5 <- bplapply(X=1:B, rseed=rseed, FUN=simulateOneSplit,  
...
if (!file.exists(resfile_n10)){
  null10 <- bplapply(X=1:B, rseed=837, FUN=simulateOneSplit,
                     nDE=nDE, sampleSize=sampleSize, bd=bd)
...
if (!file.exists(resfile_d5)){
  de5 <- bplapply(X=1:B, rseed = rseed, FUN=simulateOneSplit, 
                  nDE=nDE, sampleSize=sampleSize, bd=bd)
...
if (!file.exists(resfile_d10)){
  de10 <- bplapply(X=1:B, rseed = rseed, FUN=simulateOneSplit, 
                   nDE=nDE, sampleSize=sampleSize, bd=bd)
...
if (!file.exists(resfile_n5_uninfCov)){
  null5 <- bplapply(X=1:B, rseed=rseed, FUN=simulateOneSplit, 
                   ...
                   bd=bd, uninformativeCovariate = TRUE)
...
if (!file.exists(resfile_n10_uninfCov)){
  null10 <- bplapply(X=1:B, rseed=rseed, FUN=simulateOneSplit, 
                     nDE=nDE, sampleSize=sampleSize, 
                   bd=bd, uninformativeCovariate = TRUE)
