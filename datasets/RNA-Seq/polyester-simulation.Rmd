...
simulateOneSplit <- function(X, rseed, counts, paramPolyester, 
...
if (!file.exists(resfile_n5)){
  null5 <- bplapply(X=1:B, rseed=rseed, FUN=simulateOneSplit, counts=counts,
...
if (!file.exists(resfile_n10)){
  null10 <- bplapply(X=1:B, rseed=837, FUN=simulateOneSplit, counts=counts,
                   paramPolyester=params, nDE=nDE, sampleSize=sampleSize, bd=bd)
...
if (!file.exists(resfile_d5)){
  de5 <- bplapply(X=1:B, rseed = rseed, FUN=simulateOneSplit, counts=counts,
                   paramPolyester=params, nDE=nDE, sampleSize=sampleSize, bd=bd)
...
if (!file.exists(resfile_d10)){
  de10 <- bplapply(X=1:B, rseed = rseed, FUN=simulateOneSplit, counts=counts,
                   paramPolyester=params, nDE=nDE, sampleSize=sampleSize, bd=bd)
...
if (!file.exists(resfile_n5_uninfCov)){
  null5 <- bplapply(X=1:B, rseed=rseed, FUN=simulateOneSplit, counts=counts,
                   paramPolyester=params, nDE=nDE, sampleSize=sampleSize, 
...
if (!file.exists(resfile_n10_uninfCov)){
  null10 <- bplapply(X=1:B, rseed=rseed, FUN=simulateOneSplit, counts=counts,
                   paramPolyester=params, nDE=nDE, sampleSize=sampleSize, 
                   bd=bd, uninformativeCovariate = TRUE)
