---
title: "Yeast RNA-seq 48 sample simulation study - II"
...
For this reason, we do not include this method in the benchmark comparisons.
...
```{r}
data.frame(resFULL[resFULL$padj < 0.05,]) %>% 
  ggplot(aes(x = stat)) +
  geom_density() +
  xlab("Test Statistic") 
...
Subset on only significant DE genes (FDR < 0.05) to draw fold changes from.

```{r}
resFULL <- resFULL[resFULL$padj < 0.05,]
```

## FDR benchmarking

Build common bench design. We also add in Scott's FDR Regression (both
`nulltype = "empirical"` and `nulltype = "theoretical"`)
since our test statistics are approximately t-distributed. We also 
drop the ashq method since its assumptions are violated here.
...
simulateOneSplit <- function(X, rseed, nDE, sampleSize,
  ...
  if(nDE > 0){
    ...
    log2FC[DE] <- resFULL$log2FoldChange[sample(1:nrow(resFULL), nDE, 
                                                replace = TRUE)]
    log2FC[DE] <- pmax(1, abs(rnorm(nDE, 2)))
