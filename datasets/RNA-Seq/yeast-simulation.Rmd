...
bd <- addBMethod(bd, "scott-empirical",
                     ...
                     features = model.matrix( ~  splines::bs(ind_covariate, df = 3) - 1),
                     nulltype = 'empirical',
                     control = list(lambda = 0.01, nmids = 300, densknots = 20))
```
...
simulateOneSplit <- function(X, rseed, nDE, sampleSize,
  ...
  if(nDE > 0){
    ...
    multiplier <- ifelse(ran < 0.5, 2^log2FC, 1/(2^log2FC))
  ...
  geneExp <-  geneExp %>% dplyr::filter(!is.na(pval)) %>% dplyr::filter(pzero < 0.5)
...
if (!file.exists(resfile_n5)){
  null5 <- bplapply(X=1:B, FUN=simulateOneSplit, rseed=rseed, 
...
if (!file.exists(resfile_n10)){
  null10 <- bplapply(X=1:B, FUN=simulateOneSplit, rseed=rseed, 
                    nDE=nDE, sampleSize=sampleSize, bd=bd)
...
if (!file.exists(resfile_d5)){
  de5 <- bplapply(X=1:B, FUN=simulateOneSplit, rseed=rseed, 
                  nDE=nDE, sampleSize=sampleSize, bd=bd)
...
if (!file.exists(plotfile)){
  hists <- bplapply(X=1:B, FUN=simulateOneSplit, rseed=rseed, 
                   nDE=nDE, sampleSize=sampleSize, bd=bd, 
                   pvalHists = TRUE)
...
if(!file.exists(resfile_d10)){
  de10 <- bplapply(X=1:B, FUN=simulateOneSplit, rseed=rseed, 
                  nDE=nDE, sampleSize=sampleSize, bd=bd)
