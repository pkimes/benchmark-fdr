...
resfile_d5_uninfCov_w <- file.path(resdir, "yeastHW-results-de5-uninfCov.rds")
resfile_d10_uninfCov_w <- file.path(resdir, "yeastHW-results-de10-uninfCov.rds")
...
simulateOneSplit <- function(X, rseed, nDE, sampleSize,
  ...
  if(nDE > 0){
    # pick random set of nDE genes to add signal to
     if(strongCovariate){
      ...
      wts <- abs(covar + rnorm(length(covar)))
    }
  ...
  if (uninformativeCovariate)
    geneExp <- mutate(geneExp, ind_covariate = abs(rnorm(length(covar))))
...
if(!file.exists(resfile_d5_uninfCov_w)){
  de5 <- mclapply(X=1:B, FUN=simulateOneSplit, rseed=rseed, 
                  nDE=nDE, sampleSize=sampleSize, bd=bd,
                  uninformativeCovariate = TRUE, 
                  strongCovariate=FALSE,
                  mc.cores=nCores)
  saveRDS(de5, file = resfile_d5_uninfCov_w)
}else{
  de5 <- readRDS(file = resfile_d5_uninfCov_w)
}
```
...
if(!file.exists(resfile_d10_uninfCov_w)){
  de10 <- mclapply(X=1:B, FUN=simulateOneSplit, rseed=rseed, 
                   nDE=nDE, sampleSize=sampleSize, bd=bd,
                   uninformativeCovariate = TRUE, 
                   strongCovariate = FALSE,
                   mc.cores=nCores)
  saveRDS(de10, file = resfile_d10_uninfCov_w)
}else{
  de10 <- readRDS(file = resfile_d10_uninfCov_w)
}
```
...
plotMethodRanks(c(resfile_d5, resfile_d5_w, resfile_d10, resfile_d10_w,
                  resfile_d5_uninfCov, resfile_d5_uninfCov_w, resfile_d10_uninfCov, resfile_d10_uninfCov_w), 
