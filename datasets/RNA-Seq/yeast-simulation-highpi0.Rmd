...
resfile_d5_uw <- file.path(resdir, "yeastHU-results-de5.rds")
resfile_d10_uw <- file.path(resdir, "yeastHU-results-de10.rds")
...
resfile_d5_uninfCov_uw <- file.path(resdir, "yeastHU-results-de5-uninfCov.rds")
resfile_d10_uninfCov_uw <- file.path(resdir, "yeastHU-results-de10-uninfCov.rds")
...
simulateOneSplit <- function(X, rseed, nDE, sampleSize,
                             ...
                             weightByCovariate = TRUE){
  
  uninf_covar <- rnorm(nrow(dds_full))
  ...
  p3 <- hotelling.test(pc$x[colData(dds_test)$fake == "A", 1:4],
                       pc$x[colData(dds_test)$fake == "B", 1:4])$pval
  tries <- 0
  
  while(p1 < 0.20 || p2 < 0.20 || p3 < 0.20 && tries < 10){
  ...
  if(nDE > 0){
    ...
    if(weightByCovariate){
      wts <- rowMeans(counts(dds_test, normalize = TRUE))
    }else{
      wts <- rep(1, nrow(dds_test))
    }
...
# Sim D5: Unweighted 5v5

Here we'll repeat the above, but not explicitly weighting the DE gene selection by
the informative covariate.
...
if (!file.exists(resfile_d5_uw)){
  de5 <- mclapply(X=1:B, FUN=simulateOneSplit, rseed=rseed, 
                  nDE=nDE, sampleSize=sampleSize, bd=bd, mc.cores=nCores,
                  weightByCovariate = FALSE)
  saveRDS(de5, file = resfile_d5_uw)
}else{
  de5 <- readRDS(file = resfile_d5_uw)
}
```
...
if(!file.exists(resfile_d10_uw)){
  de10 <- mclapply(X=1:B, FUN=simulateOneSplit, rseed=rseed, 
                  nDE=nDE, sampleSize=sampleSize, bd=bd, mc.cores=nCores,
                  weightByCovariate = FALSE)
  saveRDS(de10, file = resfile_d10_uw)
}else{
  de10 <- readRDS(file = resfile_d10_uw)
}
```
...
```{r d5-uc-uw, results='hide', message=FALSE}
rseed <- 445
sampleSize <- 5
nDE <- 500

if(!file.exists(resfile_d5_uninfCov_uw)){
  de5 <- mclapply(X=1:B, FUN=simulateOneSplit, rseed=rseed, 
                  nDE=nDE, sampleSize=sampleSize, bd=bd,
                  uninformativeCovariate = TRUE, 
                  weightByCovariate=FALSE,
                  mc.cores=nCores)
  saveRDS(de5, file = resfile_d5_uninfCov_uw)
}else{
  de5 <- readRDS(file = resfile_d5_uninfCov_uw)
}
```
...
```{r d10-uc-uw, results='hide', message=FALSE}
rseed <- 868
sampleSize <- 10
nDE <- 500

if(!file.exists(resfile_d10_uninfCov_uw)){
  de10 <- mclapply(X=1:B, FUN=simulateOneSplit, rseed=rseed, 
                   nDE=nDE, sampleSize=sampleSize, bd=bd,
                   uninformativeCovariate = TRUE, 
                   weightByCovariate = FALSE,
                   mc.cores=nCores)
  saveRDS(de10, file = resfile_d10_uninfCov_uw)
}else{
  de10 <- readRDS(file = resfile_d10_uninfCov_uw)
}
```
...
plotMethodRanks(c(resfile_d5, resfile_d5_uw, resfile_d10, resfile_d10_uw,
                  resfile_d5_uninfCov, resfile_d5_uninfCov_uw, resfile_d10_uninfCov, resfile_d10_uninfCov_uw), 
                colLabels = c("5", "5 UW", "10", "10 UW",
                              "5 UI", "5 UI UW", "10 UI", "10 UI UW"), 
