...
```{r, GWAS1-download}
if(!file.exists(paste0(datdir,"BMI.SNPadjSMK.CombinedSexes.EuropeanOnly.txt"))){
  download.file(url="http://portals.broadinstitute.org/collaboration/giant/images/3/3a/BMI.SNPadjSMK.zip", 
             destfile=paste0(datdir,"BMI.SNPadjSMK.zip")) 
  unzip(paste0(datdir,"BMI.SNPadjSMK.zip"), exdir=datdir)
  file.remove(paste0(datdir,"BMI.SNPadjSMK.zip"))
...
```{r, GWAS1-refdata}
reffile <- paste0(datdir, "1000G_20101123_v3_GIANT_chr1_23_minimacnamesifnotRS_CEU_MAF0.01")
if (!file.exists(paste0(reffile, ".fam"))){
  download.file(url="http://neurogenetics.qimrberghofer.edu.au/iSECA/1000G_20101123_v3_GIANT_chr1_23_minimacnamesifnotRS_CEU_MAF0.01.zip", 
              destfile=paste0(reffile, ".zip"))
  unzip(paste0(reffile, ".zip"))
...
plink \
--bfile DATA/1000G_20101123_v3_GIANT_chr1_23_minimacnamesifnotRS_CEU_MAF0.01 \
--clump DATA/rslist.assoc \
--clump-field P \
--clump-p1 0.9999 \
--clump-p2 0.9999 \
--clump-r2 0.2 \
--clump-kb 250 \
--out DATA/plink_clump
```
...
mcols(sbN)$ind_covariate <- mcols(sbN)$ind_covar_N
plotCovariateBoxplots(sbN, alpha=0.1, nsets=6, methods=methods, trans=log2, maxNum=3) +
  ggtitle("Covariate 1: Sample size")

```

```{r, fig.width=8, fig.height=3.5}
mcols(sbN)$tstat <- bmi %>% filter(!is.na(ind_covar_N)) %>% pull(effect_size)
covariateHeatmap(sbN, alpha=0.05, covname="tstat", trans = "log1p")
covariateHeatmap(sbN, alpha=0.05, covname="ind_covar_N", trans = "log1p")

covariateHeatmap(sbN, alpha=0.05, covname="tstat", nbins=25, 
                 linePlot = TRUE, trans="log1p")
covariateHeatmap(sbN, alpha=0.05, covname="ind_covar_N", nbins=25, 
                 linePlot = TRUE, trans="log1p")
```
...
methods <- c("ashs", "lfdr", "ihw-a10", "bl-df03", "qvalue", "bh", "bonf",
             "scott-empirical", "scott-theoretical")
mcols(sbAF)$ind_covariate <- abs(mcols(sbAF)$ind_covar_AF-0.5)
plotCovariateBoxplots(sbAF, alpha=0.1, nsets=6, methods=methods, maxNum=3)+
  ggtitle("Covariate 2: Minor allele frequency")
```

```{r, fig.width=8, fig.height=3.5}
mcols(sbAF)$tstat <- bmi %>% filter(!is.na(ind_covar_AF)) %>% pull(effect_size)
covariateHeatmap(sbAF, alpha=0.05, covname="tstat", trans = "log1p")
covariateHeatmap(sbAF, alpha=0.05, covname="ind_covar_AF")

covariateHeatmap(sbAF, alpha=0.05, covname="tstat", nbins=25, 
                 linePlot = TRUE, trans = "log1p")
covariateHeatmap(sbAF, alpha=0.05, covname="ind_covar_AF", nbins=25, 
                 linePlot = TRUE)
```
