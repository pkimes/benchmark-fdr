...
bmi <- bmi[sample(1:nrow(bmi), 10000),]
```
...
resfile <- paste0("bmi_results_", nrow(bmi), ".RData")
...
p <- ggplot(bmi_results, aes(alpha, n_rejects, color=method)) +
  geom_point() +
  geom_line() +
...
# Demo on a small subset of SNPs first (remove this later)
asthma <- asthma[sample(1:nrow(asthma), 10000),]
```
...
resfile <- paste0("asthma_results_", nrow(asthma), ".RData")
if (!file.exists(resfile)){
...
# plot N rejects vs FDR cutoff
p <- ggplot(asthma_results, aes(alpha, n_rejects, color=method)) +
  geom_point() +
  geom_line() +
  xlab("FDR cutoff") + ylab("Number of rejections") +
...
# Demo on a small subset of SNPs first (remove this later)
trig <- trig[sample(1:nrow(trig), 10000),]
```
...
# use same cutoffs as for the bmi study (defined above)
resfile <- paste0("trig_results_", nrow(trig), ".RData")
if (!file.exists(resfile)){
...
# plot N rejects vs FDR cutoff
p <- ggplot(trig_results, aes(alpha, n_rejects, color=method)) +
  geom_point() +
  geom_line() +
  xlab("FDR cutoff") + ylab("Number of rejections") +
