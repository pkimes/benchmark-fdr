...
discoveries for each one. 

### Workspace setup

To run the benchmark methods on the GWAS datasets, we'll use some of the helper 
and wrapper functions from the simulated data analysis. We also create and source an analgous version of the `sim_runner` function to apply to the real data.

```{r, wkspace-setup}
# Note that swfdr package depends on R version 3.4 or higher
setwd("/n/irizarryfs01_backed_up/kkorthauer/benchmark-fdr/")
...
if (!file.exists("bmi_results.RData")){
  bmi_results <- run_benchmarks(dat=bmi, alphas=cutoffs)
  save(bmi_results, file="bmi_results.RData")
}else{
  load("bmi_results.RData")
}

# plot
ggplot(bmi, aes(alpha, n_rejects, color=method)) +
  geom_point() +
  geom_line() +
...
if (!file.exists("asthma_results.RData")){
  asthma_results <- run_benchmarks(dat=asthma, alphas=cutoffs)
  save(asthma_results, file="asthma_results.RData")
}else{
  load("asthma_results.RData")
}

# plot
ggplot(asthma_results, aes(alpha, n_rejects, color=method)) +
  geom_point() +
  geom_line() +
  xlab("FDR cutoff") + ylab("Number of rejections") +
...
# read in data
trig <- fread("jointGwasMc_TG.txt", header=TRUE)
dim(trig)
head(trig)
```
...
```{r, GWAS2-run}
# use same cutoffs as for the bmi study (defined above)
if (!file.exists("triglyceride_results.RData")){
  trig_results <- run_benchmarks(dat=trig, alphas=cutoffs)
  save(trig_results, file="triglyceride_results.RData")
}else{
  load("triglyceride_results.RData")
}


# plot
ggplot(trig_results, aes(alpha, n_rejects, color=method)) +
  geom_point() +
  geom_line() +
  xlab("FDR cutoff") + ylab("Number of rejections") +
  ggtitle("Asthma GWAS Metanalysis with Sample Size Covariate")
