...
First, we'll create an object of `BenchDesign` class to hold the data. Before 
doing so, we'll rename the relevant columns so that they will use common terms
across the different datasets. We'll also add a 'zscore' column.
...
Next, we'll add the benchmark methods to the `BenchDesign` object.

```{r, GWAS1-methods}
# This should maybe be sourced from a common script that everyone uses...
bd %<>% addBMethod(blabel = "BH", bfunc = p.adjust,
                 p = pval, method = "BH")

bd %<>% addBMethod("qv", qvalue::qvalue,
                  p = pval,
                  bpost = function(x) { x$qvalues })

bd %<>% addBMethod("ihw", IHW::ihw,
                  pvalues = pval,
...
names(assays(sb)) <- names(rowData(sb)) <- names(sb@performanceMetrics) <- "qvalue"
...
# estimate perf metrics
sb <- estimatePerformanceMetrics(sb, alpha=seq(0.01, 0.10, 0.01), addColData=TRUE)
colData( sb )
```
...
## to go here - standardized plotting functions from SummarizedBenchmarks -
## to discuss at meeting 10/19
