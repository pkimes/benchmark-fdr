...
explore the mouse dataset analysed with scDD.
...
For data processing, we follow practices and recommendations from Soneson et al., 2018. Briefly, we limit ourselves to 2-group comparisons. We only filter out genes that are expressed in 
...
*Add More detail on the experiments here*
...
strat_hist(data.frame(rowData(mousedata)), 
           pvalue="scdd_p_combined", covariate="meanExp", maxy=20,
...
```{r, fig.width=10, fig.height=3.2, message=FALSE}
strat_hist(data.frame(rowData(mousedata)), 
           pvalue="scdd_p_combined", covariate="detection", maxy=20,
...
res1 <- file.path(resdir, "sb_GSE94383_scdd_mean.rds")
...
res2 <- file.path(resdir, "sb_GSE94383_scdd_det.rds")
