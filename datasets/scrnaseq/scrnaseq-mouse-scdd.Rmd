...
datfile <- paste0(datdir,"GSE94383.rds")
...
```{r}
datfile <- paste0(datdir, "mouse_GSE94383_scdd.rds")
...
if (!file.exists(datfile)){
  ...
  mousedata <- mousedata[rowMeans(assay(mousedata, "count") > 0) > 0.01, ]
...
strat_hist(data.frame(rowData(mousedata)), 
           pvalue="scdd_p", covariate="meanExp", maxy=16,
...
```{r, fig.width=10, fig.height=3.2, message=FALSE}
strat_hist(data.frame(rowData(mousedata)), 
           pvalue="scdd_p", covariate="detection", maxy=16,
...
res1 <- paste0(resdir, "sb_GSE94383_scdd_mean.rds")
...
res2 <- paste0(resdir, "sb_GSE94383_scdd_det.rds")
