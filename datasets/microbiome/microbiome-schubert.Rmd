...
Here we download and analyze the Schubert diarrhea dataset [(Schubert et al., 2014)](http://dx.doi.org/10.1128/mBio.01021-14).
...
## Remove OTUs w/ <= min reads, samples w/ <= min reads
otu <- otu[, colSums(otu) > min_otu]
otu <- otu[rowSums(otu) > min_sample, ]
...
resfile <- file.path(resdir, paste0("schubert_results_", nrow(abun_otu), "_OTUs.rda"))
...
for (nmids in c(5, 50, 150)) {
    ...
    resfile <- file.path(resdir, paste0("schubert_summarizedBenchmark_",
