...
```{r, schubert-sb, results="hide", message=FALSE, eval=FALSE}
...
# don't plot scott theoretical for now
rmv.scott <- which(grepl("scott", colnames(sb)))

# plot nrejects by method overall and stratified by covariate
rejections_scatter(sb, as_fraction=FALSE, supplementary=FALSE)

rejection_scatter_bins(sb, covariate="ind_covariate", bins=4, supplementary=FALSE)
  
# upset plot (only plot IHW for the plotted alpha)
rmv.ihw <- which(grepl("ihw", colnames(sb)) &
                              colData(sb)$param.alpha != 0.05)
rmv.cols <- c(rmv.ihw, rmv.scott)

plotMethodsOverlap(sb[,-rmv.cols], 
                   alpha=0.05, nsets=ncol(sb)-length(rmv.cols),
                   order.by="freq", decreasing=TRUE)

# upset plot excluding "unadjusted"
plotMethodsOverlap(sb[,-c(1,rmv.cols)], 
                   alpha=0.05, nsets=ncol(sb)-length(rmv.cols)-1,
                   order.by="freq", decreasing=TRUE)
```
...
```{r, eval=FALSE}
assays(sb)[["qvalue"]][ , 15] %>% max()
sum(is.na(assays(sb)[["qvalue"]][ , 22]))
sum(is.na(assays(sb)[["qvalue"]][ , 20]))
```

Rather than de-bug this dataset (which might be weird), let's try instead to work with the obesity dataset and see if I get the same problems.
...
```{r, goodrich-sb, results="hide", message=FALSE, eval=FALSE}
for (nmids in c(5, 50, 150)){
  print(nmids)
  bd <- initializeBenchDesign(nmids=nmids)
...
```{r, goodrich-plot, results="hide", width=15, height=15, eval=FALSE}
...
Hm, looks like all of the methods run without giving NaN's with nmids = 150 or lower...
But the results still aren't right.
...
```{r, baxter-phylo, fig.width=10, fig.height=6}
#strat_hist(res, pvalue="pval", covariate="mean_abun_present", maxy=20)
#rank_scatter(res, pvalue="pval", covariate="ubiquity")
ggplot(res, aes(x=pval)) + geom_histogram() + facet_wrap(~phylum, scales = "free")
```
...
```{r, baxter-sb, results="hide", message=FALSE, eval=FALSE}
for (nmids in c(5, 50, 150)){
  print(nmids)
  bd <- initializeBenchDesign(nmids=nmids)
...
```{r, baxter-plot, results="hide", width=15, height=15, eval=FALSE}
# don't plot scott theoretical for now
rmv.scott <- which(grepl("scott", colnames(sb)))
