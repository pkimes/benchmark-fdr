...
# library(qvalue)
# library(swfdr)
  ## to use IHWpaper::scott_fdrreg wrapper, we need to install FDRreg v2.0 (only available github):   
  ## devtools::install_github(repo= 'jgscott/FDRreg', subdir='R_pkg/') 
  ## however, I couldn't get this to install. So I did the v1.0 via install.packages("FDRreg") on CRAN
# library(FDRreg)    
...
scott_fdrreg_hickswrapper <- function(unadj_p, filterstat, df=3)
  {
     ...
     Xs <- model.matrix( ~  b - 1)
     fdrreg_res <- FDRreg::FDRreg(z=qnorm(unadj_p), covars=Xs) # assumption of test statistic follow a standard normal
...
Simulate an example "null" dataset. `H` is an indicator representing the true null (0) or alternative status (1). `t_stat` is the t-test statistic, `t_dm` is the difference in means, 
`t_pval` is the $p$-value, `sd` is the pooled standard deviation ($\sigma$), which reduces to the unpooled sd in this case, 
...
readRDS(file=file.path(workingPath, "nullSims-Results.RDS"))
