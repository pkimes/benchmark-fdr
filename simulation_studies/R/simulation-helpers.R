...
#' * `t_dm`: group mean difference
#' * `t_pval`: t-test p-value
#' * `sd`: standard deviation across all samples
#' * `SE`: standard error across all samples
#'
#' @md
...
#' 
#' @param sim_df data.frame output from `du_ttest_sim` containing details of 
#'        simulated hypothesis tests
...
#' @md
#' @author Stephanie Hicks
sim_runner <- function(sim_df, alphas) { 
    ...
    ## IHW (Wolfgang)
    ihw_fdr <- IHW::ihw(pvalues = sim_df$t_pval, covariates = sim_df$sd, alpha =  0.1)
    adj_p <- IHW::adj_pvalues(ihw_fdr)
    df.ihw <- plyr::ldply(alphas, function(a){
        calculate_test_stats(sim=sim_df, adj_p = adj_p, alpha = a) })

    ## ash (Stephens)
    beta.ash = ashr::ash(betahat = sim_df$t_dm, sebetahat = sim_df$SE)
    ...
    pi0x <- swfdr::lm_pi0(pValues = sim_df$t_pval, X = sim_df$sd, smooth.df =3)
    adj_p <- p.adjust(sim_df$t_pval, method="BH")*pi0x$pi0
    df.BL <- plyr::ldply(alphas, function(a){
        calculate_test_stats(sim=sim_df, adj_p = adj_p, alpha = a) })
    
    ## Scott et al. (2015) (FDRreg)
    adj_p <- scott_fdrreg_hickswrapper(unadj_p = sim_df$t_pval, filterstat = sim_df$sd, df=3)
